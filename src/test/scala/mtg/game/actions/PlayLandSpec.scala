...
class PlayLandSpec extends SpecWithGameStateManager {
  def bePlayLandAction(land: CardDefinition): Matcher[PlayLandPriorityAction] = {
    {(playLandAction: PlayLandPriorityAction) => playLandAction.land.gameObject} ^^ beCardObject(land)
  }
