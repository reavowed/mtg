...
import mtg.game.actions.{ActivateAbilityAction, PlayLandSpecialAction, PriorityAction}
  ...
  object TakeAction {
    def unapply(string: String): Option[PriorityAction] = {
      if (string.startsWith("Play ")) {
        string.substring("Play ".length).toIntOption.flatMap(id => availableActions.ofType[PlayLandSpecialAction].find(_.land.gameObject.objectId.sequentialId == id))
