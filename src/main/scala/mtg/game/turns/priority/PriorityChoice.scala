...
import mtg.game.actions.{ActivateAbilityAction, PlayLandPriorityAction, PriorityAction}
  ...
  object TakeAction {
    def unapply(string: String): Option[PriorityAction] = {
      if (string.startsWith("Play ")) {
        string.substring("Play ".length).toIntOption.flatMap(id => availableActions.ofType[PlayLandPriorityAction].find(_.land.gameObject.objectId.sequentialId == id))
