...
case class CardObject(card: Card, objectId: ObjectId, zone: Zone, defaultController: Option[PlayerIdentifier], permanentStatus: Option[PermanentStatus]) extends GameObject {
  ...
  override def setDefaultController(newDefaultController: Option[PlayerIdentifier]): GameObject = copy(defaultController = newDefaultController)
