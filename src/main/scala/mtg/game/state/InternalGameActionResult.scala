...
case class InternalGameActionResult(childActions: Seq[GameAction], gameEvent: Option[GameEvent], logEvent: Option[LogEvent]) extends GameActionResult {
...
object InternalGameActionResult {
  ...
  implicit def gameEvent(gameEvent: GameEvent): InternalGameActionResult = InternalGameActionResult(Nil, Some(gameEvent), None)
