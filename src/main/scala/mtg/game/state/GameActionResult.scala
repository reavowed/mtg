...
object GameActionResult {
  ...
  case class MoreActions(nextActions: Seq[GameObjectAction]) extends GameActionResult
  ...
  implicit def nextAction(childAction: GameObjectAction): GameActionResult = MoreActions(Seq(childAction))
  implicit def optionalNextAction(actionOption: Option[GameObjectAction]): GameActionResult = actionOption.map(nextAction).getOrElse(Nothing)
  implicit def nextActions(childActions: Seq[GameObjectAction]): GameActionResult = MoreActions(childActions)
